...
async function handleProgramAssigned(data) {
  ...
  await sendEmail(
    ...
    `A new training program has been assigned to you by your trainer. Program ID: ${program_id}`
  );
  ...
  storeNotification(client_id, {
    ...
    message: 'Your trainer has assigned a new training program to you. Check it out!',
...
async function handleAchievementEarned(data) {
  ...
  const { achievement_id, client_id, type, title } = data;

  await sendEmail(
    `client-${client_id}@fitsync.com`,
    'Achievement Unlocked!',
    `Congratulations! You've earned a new achievement: ${title}`
  );
